...
<figure>
  <img src="01-six-components.webp" alt="왼쪽에 API server가 있고 오른쪽 열에 Workload manager, LLM engine, Model executor가 위에서 아래로 이어지며, 그 아래를 가로지르는 점선이 메인 프로세스와 워커 프로세스를 가르고, Result queue와 Input queue가 그 점선에 걸터앉아 있으며, 맨 아래 Model worker 박스에 모델과 토크나이저 로드가 적힌 도식">
