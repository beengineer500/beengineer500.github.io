...
**`--max-model-len`과 `--max-num-batched-tokens`는 둘 다 토큰 수인데 왜 따로 있는가?** 재는 대상이 다릅니다. 앞의 것은 요청 하나의 길이, 뒤의 것은 한 iteration에 담긴 요청들의 합계입니다. `1024`와 `4096`이면 컨텍스트를 꽉 채운 요청 네 개가 한 배치에 들어갈 수 있다는 뜻이고, 한 요청이 5,000 토큰을 넣으려 하면 합계 상한과는 무관하게 길이 상한에서 거절됩니다.

**청크 프리필은 종단 지연시간을 개선하지 않는데 왜 쓰는가?** 개선하는 건 ITL과 처리량이지 종단 지연시간이 아닙니다. 오히려 작은 Prefill 스텝을 여러 번 계산하는 오버헤드 때문에 종단 지연시간과 TTFT는 조금 나빠집니다. 그래서 이건 켜면 좋은 기능이 아니라 **맞바꿈**입니다. 글자가 끊기지 않고 흘러나오는 게 중요한 대화형 서비스라면 사고, 첫 응답까지의 시간이 전부인 워크로드라면 파는 쪽입니다.

**MQA가 KV 헤드를 32개에서 1개로 줄이는데 왜 표에는 25%로 적혀 있는가?** 두 숫자가 재는 게 다릅니다. `32배`는 KV 헤드 **개수의 비**이고, `25%`는 특정 구성에서 측정한 **KV 캐시 메모리 비중**입니다. 실제 캐시 크기에는 헤드 수 말고도 레이어 수, 헤드 차원, 정밀도가 같이 곱해지고, 비교하는 두 모델의 구성이 다르면 배수도 달라집니다. 자료마다 값이 다르게 나오는 게 이 때문입니다. 배수를 외우기보다 `num_key_value_heads` 비를 직접 보는 편이 확실합니다.
